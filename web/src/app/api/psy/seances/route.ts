...
export async function GET(req: NextRequest) {
  ...
  const rows = days
    ? db
        ...
        .all(`-${days}`)
    : db.prepare("SELECT * FROM psy_seances ORDER BY date DESC, created_at DESC").all();
...
export async function POST(req: NextRequest) {
  ...
  const row = db.prepare("SELECT * FROM psy_seances WHERE id = ?").get(Number(result.lastInsertRowid));
